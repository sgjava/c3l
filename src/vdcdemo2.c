...
void fillScr() {
    register uchar i;
    for (i = 0; i < 24; i++) {
        print(0, i, "| Watch how fast you can fill the screen with text. CP/M will never be as fast |");
    }
    waitKey();
}
...
void fillScrCol() {
    ...
    clearScr(32);
    for (i = 0; i < 24; i++) {
        printCol(i + 5, i , vdcAltChrSet | (i / 2 + 1), "You can do fast color text as well");
    }
    waitKey();
}
