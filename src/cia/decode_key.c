...
unsigned char decodeKey() {
	...
	static unsigned char stdKeys[11][8] = { { 0x7f, 0x0d, 0x04, 0x00, 0x00, 0x00, 0x00, 0x18 }, { '3', 'w', 'a', '4', 'z', 's', 'e',
			0x00 }, { '5', 'r', 'd', '6', 'c', 'f', 't', 'x' }, { '7', 'y', 'g', '8', 'b', 'h', 'u', 'v' }, { '9', 'i', 'j', '0',
			'm', 'k', 'o', 'n' }, { '+', 'p', 'l', '-', '.', ':', '@', ',' }, { '\\', '*', ';', 0x00, 0x00, '=', '^', '/' }, { '1',
			0x00, 0x00, '2', 0x20, 0x00, 'q', 0x00 }, { 0x00, '8', '5', 0x09, '2', '4', '7', '1' }, { 0x1b, '+', '-', 0x0a, 0x0d,
			'6', '9', '3' }, { 0x00, '0', '.', 0x05, 0x18, 0x13, 0x04, 0x00 } };

	// Key to ASCII code shifted. Unmapped keys are set to 0x00.
	static unsigned char shiftKeys[11][8] = { { 0x7f, 0x0d, 0x04, 0x00, 0x00, 0x00, 0x00, 0x18 }, { '#', 'W', 'A', '$', 'Z', 'S',
			'E', 0x00 }, { '%', 'R', 'D', '&', 'C', 'F', 'T', 'X' }, { '\'', 'Y', 'G', '(', 'B', 'H', 'U', 'V' }, { ')', 'I', 'J',
			'0', 'M', 'K', 'O', 'N' }, { '+', 'P', 'L', '-', '>', '[', '@', '<' }, { '\\', '*', ']', 0x00, 0x00, '=', '^', '?' }, {
			'!', 0x00, 0x00, '"', 0x20, 0x00, 'Q', 0x00 }, { 0x00, '8', '5', 0x09, '2', '4', '7', '1' }, { 0x1b, '+', '-', 0x0a,
			0x0d, '6', '9', '3' }, { 0x00, '0', '.', 0x05, 0x18, 0x13, 0x04, 0x00 } };
	unsigned char i = 0;
	...
	if ((ciaKeyScan[1] != 0xff) || (ciaKeyScan[6] != 0xff)) {
		...
		if (lsCol < 7) {
			...
			while ((i < 11) && (ciaKeyScan[i] == 0xff)) {
				...
				if (((i == 0) && (ciaKeyScan[1] == 0x7f)) || ((i == 5) && ciaKeyScan[6] == 0xef)) {
					i++;
				}
				i++;
			}
